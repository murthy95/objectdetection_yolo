...
from yolov1 import yolonet_classification
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    data = datasets.CIFAR100(args.data, train=True, 
                            transform=transforms.Compose([transforms.RandomResizedCrop(448),
                                                          transforms.RandomHorizontalFlip(),
                                                          transforms.ToTensor()
                                                          ]),
                            download=download)
    train_dataset, val_dataset = torch.utils.data.random_split(data, lengths= [int(0.7*data.__len__()), int(0.3*data.__len__())])
