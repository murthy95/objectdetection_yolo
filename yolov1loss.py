	...
	def forward(self, x, labels):
	  ...
	  args x 		:output from the network tensor of shape 7, 7, 20 
		...
		onehotlabels = torch.zeros([labels.shape[0], 20, labels.shape[2],
														labels.shape[2]], device=self.device).scatter(1, class_labels, object_mask)
		loss = torch.mean((class_predictions - onehotlabels)**2)
		...
		box_confs = torch.cat([box1[:, -1, :, :].unsqueeze(1), box2[:, -1, :, :].unsqueeze(1)], 1)
		argmax_conf = torch.argmax(box_confs, 1)
		box1_mask = object_mask * ( argmax_conf == 0 ).type(torch.float)
		box2_mask = object_mask * ( argmax_conf == 1 ).type(torch.float)
		
		argmax_conf = argmax_conf.type(torch.float)
		final_box_coords = (1-argmax_conf)*box1_coords + argmax_conf*box2_coords
		loss += self.lambda_coord * (torch.mean(torch.matrix_power(
		...
		loss += torch.mean(torch.matrix_power(object_mask*final_box_obj - object_mask, 2))

		#no obj loss
		loss += self.lambda_noobj*torch.mean(torch.matrix_power((1-object_mask)*final_box_obj - object_mask, 2))
