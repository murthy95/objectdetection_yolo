...
class yolov1loss(nn.Module):
	...
	def forward(self, x, labels):
	  ...
	  args x 		:output from the network tensor of shape 7, 7, 20 
		...
		box1_confs = torch.sigmoid(box1_confs)
		box2_confs = torch.sigmoid(box2_confs)
		box1_dims = torch.sqrt(torch.sigmoid(box1_dims))
		box2_dims = torch.sqrt(torch.sigmoid(box2_dims))
		label_dims = torch.sqrt(label_dims)
		box_confs = torch.cat([box1_confs, box2_confs], 1)
		argmax_conf = torch.argmax(box_confs, 1, keepdim=True)
		...
		#no obj loss
		loss += self.lambda_noobj*torch.mean(((1-object_mask)*final_box_obj - object_mask)**2)
